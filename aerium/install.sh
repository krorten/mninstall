...
cd ~
echo "***********************************************************"
echo "* Welcome to Kayroons AERIUMX masternode install script *"
echo "*                                                         *"                                                        
...
sudo ufw allow ssh
sudo ufw allow 35407/tcp
echo "y" | sudo ufw enable
echo && echo "${bold}Firewall installed and enabled!${regular}"

echo && echo
echo "${bold}Downloading and installing AERIUM Files${regular}"
...
rm AeriumX-2.0.0-aarch64-linux-gnu.zip
chmod +x aeriumxd
sudo mv aeriumxd /usr/local/bin
...
mkdir -p .aeriumx
sudo touch .aeriumx/aeriumx.conf
echo '
rpcuser='$rpcuser'
...
masternode=1
' | sudo -E tee /root/.aeriumx/aeriumx.conf
sleep 3
echo && echo "${bold}Starting AERIUM Deamon...${regular}"
aeriumxd stop
sleep 10
aeriumxd -deamon &
sleep 3
cd /root/.aeriumx/
sleep 3
echo && echo "${bold}Checking AERIUM Deamon...${regular}"
aeriumxd getinfo
