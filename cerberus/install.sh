...
cd ~
echo "***********************************************************"
echo "* Welcome to Kayroons CERBERUS masternode install script  *"
echo "*                                                         *"                                                        
...
echo && echo
echo "${bold}Downloading and installing CERBERUS Files${regular}"
mkdir tempCBS
chmod -R 777 tempCBS
sudo git clone https://github.com/cerberuscore/cerberus.git tempCBS
cd tempCBS
...
sudo make install
cd --
mkdir .cerberuscore && chmod 777 .cerberuscore
sudo cp tempCBS/src/cerberus{d,-cli} /usr/local/bin
...
sudo touch .cerberuscore/cerberus.conf
echo '
rpcuser='$rpcuser'
...
masternode=1
' | sudo -E tee /root/.cerberuscore/cerberus.conf
sleep 3
echo && echo "${bold}Starting CERBERUS Deamon...${regular}"
cerberus-cli stop
sleep 10
cerberusd -deamon &
sleep 3

echo && echo "${bold}Checking CERBERUS Deamon...${regular}"
